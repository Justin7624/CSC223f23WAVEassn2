...
#
#   STUDENT NOTE: 
#       I have completed the necessary documentation sections
#       
#       I copied and pasted each wave description as a documentation comment as necessary to each
#           function.
#
#       In genTri(), I used similar code as in genRiseSaw() and genFallSaw() to create a triangle wave. 
#           The former for the first half of samples, the latter for the second half.
#
#       In genCosine(), I used the math.cos() function instead of math.sin() to create a cosine wave.
#           Because of this, I had to set startRadians to 0.
#
#       In genSquare(), I modified and used similar code from genRiseSaw() and split samples into two,
#           ensuring that each side had the appropriate value.
#
#       In genPulse(), I used similar code as in genSquare() and used dutyCycle instead.
#
#       In genFallSaw(), I used similar code as in genRiseSaw() but made stepSize negative, curvalue positive, 
#           and curvalue was being increased instead of decreasing by stepsize.
#
...
def genSamples(wavetype, samples, wavecol, table2D, dutyCycle,
    ...
    def genTri():
        '''
        Triangle wave starting at lowest point at time 0.
        Signal rises linearly from min value to max value at halfway
        sample, then falls from max to min by the end.
        '''
        stepsize = round((1.0 - -1.0) / (samples // 2.0), 10)   # distance / steps up from 0
        curvalue = round(-1.0, 10)
        for step in range(0, (samples // 2)):
            intvalue = __scalePercentToWavRange__(curvalue)
            table2D[step][wavecol] = intvalue
            curvalue += stepsize

        for step in range((samples // 2), samples):
            intvalue = __scalePercentToWavRange__(curvalue)
            table2D[step][wavecol] = intvalue
            curvalue -= stepsize
    ...
    def genCosine():
        ...
        '''
        Cosine wave starting at highest point at time 0.
        radians([0,360] degrees for 1 complete cycle
        https://www.mathopenref.com/triggraphcosine.html
        '''
    ...
    def genSquare():
        '''
        Square wave starting at lowest point at time 0.
        This is identical to a pulse with 0.5 DutyCycle.
        '''
    ...
    def genPulse():
        '''
        Pulse wave starting at min level at time 0.
        Final samples are at max level for the DutyCycle.
        '''

        stepsize = 0
        curvalue = -1.0
        for step in range(0, math.floor(samples - (samples * dutyCycle))):
            intvalue = __scalePercentToWavRange__(curvalue)
            table2D[step][wavecol] = intvalue
            curvalue += stepsize
        ...
        curvalue = 1.0
        for step in range(math.floor(samples - (samples * dutyCycle)), samples):
            intvalue = __scalePercentToWavRange__(curvalue)
            table2D[step][wavecol] = intvalue
            curvalue += stepsize
    def genRiseSaw():
    ...
    def genFallSaw():
        ...
        '''
        Falling sawtooth starting at HIGH point at time 0.
        Signal falls linearly from max value to min value at end.
        '''
